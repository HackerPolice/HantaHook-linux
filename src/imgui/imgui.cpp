...
static bool
DataTypeApplyOpFromText(const char *buf, const char *initial_value_buf, ImGuiDataType data_type, void *data_ptr,
                        const char *format);
...
// Template widget behaviors
	...
	static bool DragBehaviorT(ImGuiDataType data_type, TYPE *v, float v_speed, const TYPE v_min, const TYPE v_max,
	                          const char *format, float power);

	template<typename TYPE, typename SIGNEDTYPE, typename FLOATTYPE>
	static bool
	SliderBehaviorT(const ImRect &bb, ImGuiID id, ImGuiDataType data_type, TYPE *v, const TYPE v_min, const TYPE v_max,
...
ImGuiStyle::ImGuiStyle()
{
	...
	DisplayWindowPadding = ImVec2(20,
	                              20);    // Window positions are clamped to be visible within the display area by at least this amount. Only covers regular windows.
...
ImGuiIO::ImGuiIO()
{
	...
	for (int i = 0;
	     i < IM_ARRAYSIZE(MouseDownDuration); i++) { MouseDownDuration[i] = MouseDownDurationPrev[i] = -1.0f; }
...
static void SetCursorPosYAndSetupDummyPrevLine(float pos_y, float line_height)
{
	...
	window->DC.CursorPosPrevLine.y = window->DC.CursorPos.y -
	                                 line_height;      // Setting those fields so that SetScrollHere() can properly function after the end of our clipper usage.
	window->DC.PrevLineHeight = (line_height -
	                             GImGui->Style.ItemSpacing.y);    // If we end up needing more accurate data (to e.g. use SameLine) we may as well make the clipper have a fourth step to let user process and display the last item in their list.
...
bool ImGui::FocusableItemRegister(ImGuiWindow *window, ImGuiID id, bool tab_stop)
{
	...
	const bool allow_keyboard_focus =
			(window->DC.ItemFlags & (ImGuiItemFlags_AllowKeyboardFocus | ImGuiItemFlags_Disabled)) ==
	...
	if (tab_stop && (g.ActiveId == id) && window->FocusIdxAllRequestNext == INT_MAX &&
		...
		window->FocusIdxTabRequestNext = window->FocusIdxTabCounter + (g.IO.KeyShift ? (allow_keyboard_focus ? -1 : 0)
		                                                                             : +1);
...
void ImGui::NavUpdateWindowingList()
{
	...
	Begin("###NavWindowingList", NULL,
	      ImGuiWindowFlags_NoTitleBar | ImGuiWindowFlags_NoFocusOnAppearing | ImGuiWindowFlags_NoNav |
...
#undef NAV_MAP_KEY
	...
	if (g.NavMoveRequest && (g.NavMoveResultLocal.ID != 0 || g.NavMoveResultOther.ID != 0)) {
		...
		if (result != &g.NavMoveResultOther && g.NavMoveResultOther.ID != 0 &&
			...
			if ((g.NavMoveResultOther.DistBox < result->DistBox) || (g.NavMoveResultOther.DistBox == result->DistBox &&
	...
	if (nav_keyboard_active && g.NavMoveDir == ImGuiDir_None && g.NavWindow &&
	    !(g.NavWindow->Flags & ImGuiWindowFlags_NoNavInputs) && !g.NavWindowingTarget && g.NavLayer == 0) {
		...
		if ((page_up_held && !page_down_held) || (page_down_held && !page_up_held)) {
			if (window->DC.NavLayerActiveMask == 0x00 && window->DC.NavHasScroll) {
				...
				const float page_offset_y = ImMax(0.0f,
				                                  window->InnerClipRect.GetHeight() - window->CalcFontSize() * 1.0f +
	...
	if (g.HoveredWindow && !g.HoveredWindow->Collapsed && (g.IO.MouseWheel != 0.0f || g.IO.MouseWheelH != 0.0f)) {
		...
		if (g.IO.MouseWheel != 0.0f) {
			if (g.IO.KeyCtrl && g.IO.FontAllowUserScaling) {
				...
				scroll_amount = (float) (int) ImMin(scroll_amount, (scroll_window->ContentsRegionRect.GetHeight() +
				                                                    scroll_window->WindowPadding.y * 2.0f) * 0.67f);
...
void ImGui::Render()
{
	...
	windows_to_render_front_most[0] = (g.NavWindowingTarget &&
	...
	for (int n = 0; n < IM_ARRAYSIZE(windows_to_render_front_most); n++) {
		if (windows_to_render_front_most[n] && IsWindowActiveAndVisible(
				windows_to_render_front_most[n])) { // NavWindowingTarget is always temporarily displayed as the front-most window
...
void ImGui::ClosePopupsOverWindow(ImGuiWindow *ref_window)
{
	...
	if (n <
	    g.OpenPopupStack.Size) { // This test is not required but it allows to set a convenient breakpoint on the block below
		ClosePopupToLevel(n);
	}
}
...
static void CheckStacksSize(ImGuiWindow *window, bool write)
{
	...
	}    // Too few or too many PopID()/TreePop()
	{
		...
		if (write) { *p_backup = current; } else IM_ASSERT(*p_backup == current && "BeginGroup/EndGroup Mismatch!");
		p_backup++;
	...
	}    // Too few or too many PopStyleVar()
	{
		...
		if (write) { *p_backup = current; } else IM_ASSERT(*p_backup >= current && "PushFont/PopFont Mismatch!");
		p_backup++;
...
{
	ImVec2 sz;
	sz.x = (float) (int) ((window->SizeContentsExplicit.x != 0.0f) ? window->SizeContentsExplicit.x : (
			window->DC.CursorMaxPos.x - window->Pos.x + window->Scroll.x));
	sz.y = (float) (int) ((window->SizeContentsExplicit.y != 0.0f) ? window->SizeContentsExplicit.y : (
...
static ImVec2 CalcSizeAutoFit(ImGuiWindow *window, const ImVec2 &size_contents)
{
	...
	if (window->Flags & ImGuiWindowFlags_Tooltip) {
		...
		ImVec2 size_auto_fit = ImClamp(size_contents, size_min,
                          ...
                          ImU32 resize_grip_col[4])
{
	...
	for (int resize_grip_n = 0; resize_grip_n < resize_grip_count; resize_grip_n++) {
		...
		if (held && g.IO.MouseDoubleClicked[0] && resize_grip_n == 0) {
			...
			ImVec2 corner_target = g.IO.MousePos - g.ActiveIdClickOffset + resize_rect.GetSize() *
...
bool ImGui::Begin(const char *name, bool *p_open, ImGuiWindowFlags flags)
{
	...
	if (window_just_created) {
		ImVec2 size_on_first_use = (g.NextWindowData.SizeCond != 0) ? g.NextWindowData.SizeVal : ImVec2(0.0f,
	...
	if (first_begin_of_the_frame) {
		...
		window->WindowBorderSize = (flags & ImGuiWindowFlags_ChildWindow) ? style.ChildBorderSize : ((flags &
		...
		if ((flags & ImGuiWindowFlags_AlwaysAutoResize) && !window->Collapsed) {
			...
			if (!window_size_x_set_by_api && window->AutoFitFramesX > 0) {
				window->SizeFull.x = size_full_modified.x = window->AutoFitOnlyGrows ? ImMax(window->SizeFull.x,
				                                                                             size_auto_fit.x)
				                                                                     : size_auto_fit.x;
			}
			if (!window_size_y_set_by_api && window->AutoFitFramesY > 0) {
				window->SizeFull.y = size_full_modified.y = window->AutoFitOnlyGrows ? ImMax(window->SizeFull.y,
				                                                                             size_auto_fit.y)
				                                                                     : size_auto_fit.y;
			}
			if (!window->Collapsed) {
		...
		if (!(flags & ImGuiWindowFlags_ChildWindow)) {
			if (!window_pos_set_by_api && window->AutoFitFramesX <= 0 && window->AutoFitFramesY <= 0 &&
			    g.IO.DisplaySize.x > 0.0f && g.IO.DisplaySize.y >
		...
		window->FocusIdxAllRequestCurrent = (window->FocusIdxAllRequestNext == INT_MAX ||
		                                     window->FocusIdxAllCounter == -1) ? INT_MAX :
		                                    (window->FocusIdxAllRequestNext + (window->FocusIdxAllCounter + 1)) %
		                                    (window->FocusIdxAllCounter + 1);
		window->FocusIdxTabRequestCurrent = (window->FocusIdxTabRequestNext == INT_MAX ||
		                                     window->FocusIdxTabCounter == -1) ? INT_MAX :
		...
		const bool dim_bg_for_modal =
				(flags & ImGuiWindowFlags_Modal) && window == GetFrontMostPopupModal() && window->HiddenFrames <= 0;
		...
		if (window->Collapsed) {
			...
			// Title bar
			ImU32 title_bar_col = GetColorU32(
					window->Collapsed ? ImGuiCol_TitleBgCollapsed : title_bar_is_highlight ? ImGuiCol_TitleBgActive
					                                                                       : ImGuiCol_TitleBg);
			...
			if (!(flags & ImGuiWindowFlags_NoResize)) {
				for (int resize_grip_n = 0; resize_grip_n < resize_grip_count; resize_grip_n++) {
					...
					window->DrawList->PathLineTo(corner + grip.InnerDir *
					                                      ((resize_grip_n & 1) ? ImVec2(window_border_size,
					                                                                    grip_draw_size) : ImVec2(
							                                      grip_draw_size, window_border_size)));
					window->DrawList->PathLineTo(corner + grip.InnerDir * ((resize_grip_n & 1) ? ImVec2(grip_draw_size,
					                                                                                    window_border_size)
					                                                                           : ImVec2(
									window_border_size, grip_draw_size)));
		...
		window->ContentsRegionRect.Min.y =
				window->Pos.y - window->Scroll.y + window->WindowPadding.y + window->TitleBarHeight() +
				window->MenuBarHeight();
		window->ContentsRegionRect.Max.x = window->Pos.x - window->Scroll.x - window->WindowPadding.x +
		                                   (window->SizeContentsExplicit.x != 0.0f ? window->SizeContentsExplicit.x : (
				                                   window->Size.x - window->ScrollbarSizes.x));
		window->ContentsRegionRect.Max.y = window->Pos.y - window->Scroll.y - window->WindowPadding.y +
		...
		if (!(flags & ImGuiWindowFlags_NoTitleBar)) {
			...
			float pad_left = (flags & ImGuiWindowFlags_NoCollapse) ? style.FramePadding.x : (style.FramePadding.x +
			                                                                                 g.FontSize +
			                                                                                 style.ItemInnerSpacing.x);
			float pad_right = (p_open == NULL) ? style.FramePadding.x : (style.FramePadding.x + g.FontSize +
			                                                             style.ItemInnerSpacing.x);
			...
			clip_rect.Max.x = window->Pos.x + window->Size.x - (p_open ? title_bar_rect.GetHeight() - 3
        ...
        */
		...
		window->InnerClipRect.Min.x = ImFloor(0.5f + window->InnerMainRect.Min.x + ImMax(0.0f,
		                                                                                 ImFloor(window->WindowPadding.x *
		                                                                                         0.5f -
		                                                                                         window->WindowBorderSize)));
		window->InnerClipRect.Min.y = ImFloor(0.5f + window->InnerMainRect.Min.y);
		window->InnerClipRect.Max.x = ImFloor(0.5f + window->InnerMainRect.Max.x - ImMax(0.0f,
		                                                                                 ImFloor(window->WindowPadding.x *
		                                                                                         0.5f -
		                                                                                         window->WindowBorderSize)));
	...
	if (flags & ImGuiWindowFlags_ChildWindow) {
		...
		if (!(flags & ImGuiWindowFlags_AlwaysAutoResize) && window->AutoFitFramesX <= 0 &&
			...
			window->Collapsed |= (window->OuterRectClipped.Min.x >= window->OuterRectClipped.Max.x ||
	...
	if (horizontal) {
		...
		window_rounding_corners =
				(((window->Flags & ImGuiWindowFlags_NoTitleBar) && !(window->Flags & ImGuiWindowFlags_MenuBar))
				 ? ImDrawCornerFlags_TopRight : 0) | (other_scrollbar ? 0 : ImDrawCornerFlags_BotRight);
	}
...
bool ImGui::IsWindowNavFocusable(ImGuiWindow *window)
{
	...
	return window->Active && window == window->RootWindow &&
	       (!(window->Flags & ImGuiWindowFlags_NoNavFocus) || window == g.NavWindow);
}
...
bool ImGui::ButtonBehavior(const ImRect &bb, ImGuiID id, bool *out_hovered, bool *out_held, ImGuiButtonFlags flags)
{
	...
	if ((flags &
	     (ImGuiButtonFlags_PressedOnClickRelease | ImGuiButtonFlags_PressedOnClick | ImGuiButtonFlags_PressedOnRelease |
...
bool ImGui::ButtonEx(const char *label, const ImVec2 &size_arg, ImGuiButtonFlags flags)
{
	...
	if ((flags & ImGuiButtonFlags_AlignTextBaseLine) && style.FramePadding.y <
	                                                    window->DC.CurrentLineTextBaseOffset) { // Try to vertically align buttons that are smaller/have no padding so that text baseline matches (bit hacky, since it shouldn't be a flag)
...
bool ImGui::ArrowButtonEx(const char *str_id, ImGuiDir dir, ImVec2 size, ImGuiButtonFlags flags)
{
	...
	RenderArrow(bb.Min + ImVec2(ImMax(0.0f, size.x - g.FontSize - g.Style.FramePadding.x),
	                            ImMax(0.0f, size.y - g.FontSize - g.Style.FramePadding.y)), dir);
...
bool ImGui::CloseButton(ImGuiID id, const ImVec2 &pos, float radius)
{
	...
	if (hovered) {
		window->DrawList->AddCircleFilled(center, ImMax(2.0f, radius), GetColorU32(
...
bool ImGui::TreeNodeBehavior(ImGuiID id, ImGuiTreeNodeFlags flags, const char *label, const char *label_end)
{
	...
	const float text_base_offset_y = ImMax(padding.y,
	...
	const ImRect interact_bb = display_frame ? frame_bb : ImRect(frame_bb.Min.x, frame_bb.Min.y,
	...
	ImGuiButtonFlags button_flags = ImGuiButtonFlags_NoKeyModifiers |
	                                ((flags & ImGuiTreeNodeFlags_AllowItemOverlap) ? ImGuiButtonFlags_AllowItemOverlap
	                                                                               : 0);
	if (!(flags & ImGuiTreeNodeFlags_Leaf)) {
...
bool ImGui::CollapsingHeader(const char *label, bool *p_open, ImGuiTreeNodeFlags flags)
{
	...
	bool is_open = TreeNodeBehavior(id, flags | ImGuiTreeNodeFlags_CollapsingHeader |
	...
	if (p_open) {
		...
		if (CloseButton(window->GetID((void *) (intptr_t)(id + 1)),
		                ImVec2(ImMin(window->DC.LastItemRect.Max.x, window->ClipRect.Max.x) - g.Style.FramePadding.x -
		                       button_sz, window->DC.LastItemRect.Min.y + g.Style.FramePadding.y + button_sz),
		                button_sz)) {
			*p_open = false;
		}
...
void ImGui::BulletTextV(const char *fmt, va_list args)
{
	...
	const float line_height = ImMax(ImMin(window->DC.CurrentLineHeight, g.FontSize + g.Style.FramePadding.y * 2),
	                                g.FontSize);
	const ImRect bb(window->DC.CursorPos, window->DC.CursorPos + ImVec2(g.FontSize +
	                                                                    (label_size.x > 0.0f ? (label_size.x +
...
static float GetMinimumStepAtDecimalPrecision(int decimal_precision)
{
	...
	return (decimal_precision >= 0 && decimal_precision < 10) ? min_steps[decimal_precision] : ImPow(10.0f,
...
static inline float
SliderBehaviorCalcRatioFromValue(ImGuiDataType data_type, TYPE v, TYPE v_min, TYPE v_max, float power,
...
template<typename TYPE, typename SIGNEDTYPE, typename FLOATTYPE>
static bool ImGui::SliderBehaviorT(const ImRect &bb, ImGuiID id, ImGuiDataType data_type, TYPE *v, const TYPE v_min,
                                   const TYPE v_max, const char *format, float power, ImGuiSliderFlags flags)
{
	...
	const float slider_sz = is_horizontal ? (bb.GetWidth() - grab_padding * 2.0f) : (bb.GetHeight() -
	                                                                                 grab_padding * 2.0f);
	...
	if (g.ActiveId == id) {
		...
		if (g.ActiveIdSource == ImGuiInputSource_Mouse) {
			...
			if (g.NavActivatePressedId == id && !g.ActiveIdIsJustActivated) {
				...
				clicked_t = SliderBehaviorCalcRatioFromValue<TYPE, FLOATTYPE>(data_type, *v, v_min, v_max, power,
				                                                              linear_zero_pos);
	...
	// Draw
	float grab_t = SliderBehaviorCalcRatioFromValue<TYPE, FLOATTYPE>(data_type, *v, v_min, v_max, power,
	                                                                 linear_zero_pos);
	if (!is_horizontal) {
		grab_t = 1.0f - grab_t;
	}
	...
	ImRect grab_bb;
	if (is_horizontal) {
		grab_bb = ImRect(grab_pos - grab_sz * 0.5f, bb.Min.y + grab_padding, grab_pos + grab_sz * 0.5f,
		                 bb.Max.y - grab_padding);
	} else {
		grab_bb = ImRect(bb.Min.x + grab_padding, grab_pos - grab_sz * 0.5f, bb.Max.x - grab_padding,
                           ...
                           const void *v_max, const char *format, float power, ImGuiSliderFlags flags)
{
	...
	const ImU32 frame_col = GetColorU32(
			g.ActiveId == id ? ImGuiCol_FrameBgActive : g.HoveredId == id ? ImGuiCol_FrameBgHovered : ImGuiCol_Border);
	RenderNavHighlight(bb, id);
...
template<typename TYPE, typename SIGNEDTYPE, typename FLOATTYPE>
static bool ImGui::DragBehaviorT(ImGuiDataType data_type, TYPE *v, float v_speed, const TYPE v_min, const TYPE v_max,
                                 const char *format, float power)
{
	ImGuiContext &g = *GImGui;
	...
	if (g.ActiveIdSource == ImGuiInputSource_Mouse && IsMousePosValid() &&
		...
		int decimal_precision = (data_type == ImGuiDataType_Float || data_type == ImGuiDataType_Double)
		                        ? ImParseFormatPrecision(format, 3) : 0;
...
bool
ImGui::DragBehavior(ImGuiID id, ImGuiDataType data_type, void *v, float v_speed, const void *v_min, const void *v_max,
                    const char *format, float power)
{
	ImGuiContext &g = *GImGui;
	if (g.ActiveId == id) {
...
bool
ImGui::DragFloat(const char *label, float *v, float v_speed, float v_min, float v_max, const char *format, float power)
{
...
bool ImGui::RadioButton(const char *label, bool active)
{
	...
	const ImRect check_bb(window->DC.CursorPos, window->DC.CursorPos +
                        ...
                        ImGuiTextEditCallback callback, void *user_data)
{
	...
	const bool focus_requested = FocusableItemRegister(window, id, (flags & (ImGuiInputTextFlags_CallbackCompletion |
	...
	if (focus_requested || user_clicked || user_scrolled || user_nav_input_start) {
		if (g.ActiveId != id) {
			...
			edit_state.Text.resize(buf_size +
			                       1);        // wchar count <= UTF-8 count. we use +1 to make sure that .Data isn't NULL so it doesn't crash.
			edit_state.InitialText.resize(
					buf_size + 1); // UTF-8. we use +1 to make sure that .Data isn't NULL so it doesn't crash.
			ImStrncpy(edit_state.InitialText.Data, buf, edit_state.InitialText.Size);
			const char *buf_end = NULL;
			edit_state.CurLenW = ImTextStrFromUtf8(edit_state.Text.Data, edit_state.Text.Size, buf, NULL, &buf_end);
			edit_state.CurLenA = (int) (buf_end -
			                            buf); // We can't get the result from ImFormatString() above because it is not UTF-8 aware. Here we'll cut off malformed UTF-8.
	...
	if (g.ActiveId == id && !g.ActiveIdIsJustActivated && !clear_active_id) {
		...
		const bool is_cut = ((is_shortcut_key && IsKeyPressedMap(ImGuiKey_X)) ||
		                     (is_shift_key_only && IsKeyPressedMap(ImGuiKey_Delete))) && is_editable && !is_password &&
		...
		const bool is_paste = ((is_shortcut_key && IsKeyPressedMap(ImGuiKey_V)) ||
		                       (is_shift_key_only && IsKeyPressedMap(ImGuiKey_Insert))) && is_editable;
		...
		const bool is_redo = ((is_shortcut_key && IsKeyPressedMap(ImGuiKey_Y)) ||
		                      (is_osx_shift_shortcut && IsKeyPressedMap(ImGuiKey_Z))) && is_editable && is_undoable;
		...
		if (IsKeyPressedMap(ImGuiKey_LeftArrow)) {
			...
			if (io.SetClipboardTextFn) {
				...
				const int ie = edit_state.HasSelection() ? ImMax(edit_state.StbState.select_start,
				                                                 edit_state.StbState.select_end) : edit_state.CurLenW;
			...
			if ((flags & (ImGuiInputTextFlags_CallbackCompletion | ImGuiInputTextFlags_CallbackHistory |
				...
				if (event_flag) {
					...
					if (callback_data.CursorPos != utf8_cursor_pos) {
						edit_state.StbState.cursor = ImTextCountCharsFromUtf8(callback_data.Buf, callback_data.Buf +
					...
					if (callback_data.SelectionEnd != utf8_selection_end) {
						edit_state.StbState.select_end = ImTextCountCharsFromUtf8(callback_data.Buf, callback_data.Buf +
	...
	if (g.ActiveId == id || is_currently_scrolling) {
		...
		bool cursor_is_visible = (!g.IO.OptCursorBlink) || (g.InputTextState.CursorAnim <= 0.0f) ||
...
bool
ImGui::InputScalar(const char *label, ImGuiDataType data_type, void *data_ptr, const void *step, const void *step_fast,
                   const char *format, ImGuiInputTextFlags extra_flags)
{
	...
	if (step != NULL) {
		...
		if (InputText("", buf, IM_ARRAYSIZE(buf),
			...
			value_changed = DataTypeApplyOpFromText(buf, g.InputTextState.InitialText.Data, data_type, data_ptr,
			                                        format);
		}
		PopItemWidth();
...
bool ImGui::MenuItem(const char *label, const char *shortcut, bool selected, bool enabled)
{
	...
	if (window->DC.LayoutType == ImGuiLayoutType_Horizontal) {
		...
		PopStyleVar();
		window->DC.CursorPos.x += (float) (int) (style.ItemSpacing.x * (-1.0f +
...
bool ImGui::BeginMenu(const char *label, bool enabled)
{
	...
	if (menu_is_open) {
		...
		ImGuiWindowFlags flags =
				...
				ImGuiWindowFlags_NoSavedSettings |
				((window->Flags & (ImGuiWindowFlags_Popup | ImGuiWindowFlags_ChildMenu)) ? ImGuiWindowFlags_ChildMenu |
				                                                                           ImGuiWindowFlags_ChildWindow
...
void ImGui::ColorTooltip(const char *text, const float *col, ImGuiColorEditFlags flags)
{
	...
	ColorButton("##preview", ImVec4(col[0], col[1], col[2], col[3]), (flags & (ImGuiColorEditFlags_NoAlpha |
...
void
ImGui::RenderColorRectWithAlphaCheckerboard(ImVec2 p_min, ImVec2 p_max, ImU32 col, float grid_step, ImVec2 grid_off,
...
bool ImGui::ColorEdit4(const char *label, float col[4], ImGuiColorEditFlags flags)
{
	...
	if ((flags & (ImGuiColorEditFlags_RGB | ImGuiColorEditFlags_HSV)) != 0 &&
		...
		const float w_item_one = ImMax(1.0f,
		                               (float) (int) ((w_items_all - (style.ItemInnerSpacing.x) * (components - 1)) /
		                                              (float) components));
		const float w_item_last = ImMax(1.0f, (float) (int) (w_items_all - (w_item_one + style.ItemInnerSpacing.x) *
		                                                                   (components - 1)));
		...
		for (int n = 0; n < components; n++) {
			...
			if (flags & ImGuiColorEditFlags_Float) {
				value_changed = value_changed_as_float = value_changed | DragFloat(ids[n], &f[n], 1.0f / 255.0f, 0.0f,
				                                                                   hdr ? 0.0f : 1.0f,
	...
	if (!(flags & ImGuiColorEditFlags_NoSmallPreview)) {
		...
		if (BeginPopup("picker")) {
			...
			ImGuiColorEditFlags picker_flags_to_forward =
					ImGuiColorEditFlags__DataTypeMask | ImGuiColorEditFlags__PickerMask | ImGuiColorEditFlags_HDR |
...
bool ImGui::ColorPicker4(const char *label, float col[4], ImGuiColorEditFlags flags, const float *ref_col)
{
	...
	float sv_picker_size = ImMax(bars_width * 1, CalcItemWidth() - (alpha_bar ? 2 : 1) * (bars_width +
	...
	if (!(flags & ImGuiColorEditFlags_NoSidePreview)) {
		...
		ColorButton("##current", col_v4, (flags & (ImGuiColorEditFlags_HDR | ImGuiColorEditFlags_AlphaPreview |
		                                           ImGuiColorEditFlags_AlphaPreviewHalf |
		...
		if (ref_col != NULL) {
			...
			if (ColorButton("##original", ref_col_v4, (flags &
	...
	if (flags & ImGuiColorEditFlags_PickerHueWheel) {
		...
		sv_cursor_pos.x = ImClamp((float) (int) (picker_pos.x + ImSaturate(S) * sv_picker_size + 0.5f),
		                          picker_pos.x + 2, picker_pos.x + sv_picker_size -
...
void ImGui::Separator()
{
	...
	ItemSize(ImVec2(0.0f,
	                0.0f)); // NB: we don't provide our width so that it doesn't get feed back into AutoFit, we don't provide height to not alter layout.
	if (!ItemAdd(bb, 0)) {
		if (window->DC.ColumnsSet) {
			PushColumnClipRect();
		}
...
void ImGui::EndGroup()
{
	...
	if (!group_data.BackupActiveIdIsAlive && g.ActiveIdIsAlive &&
...
void ImGui::BeginColumns(const char *str_id, int columns_count, ImGuiColumnsFlags flags)
{
	...
	const float content_region_width = (window->SizeContentsExplicit.x != 0.0f) ? (window->SizeContentsExplicit.x) : (
					...
					while (clipper.Step()) {
						for (int prim = clipper.DisplayStart, vtx_i = elem_offset + clipper.DisplayStart * 3;
							...
							if (ImGui::IsItemHovered()) {
								...
								overlay_draw_list->AddPolyline(triangles_pos, 3, IM_COL32(255, 255, 0, 255), true,
								                               1.0f);
